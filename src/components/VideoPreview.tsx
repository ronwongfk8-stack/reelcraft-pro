...
import { audioEngine, beginGlobalExport, endGlobalExport } from '../utils/audioSynthesizer';
...
export const VideoPreview: React.FC<VideoPreviewProps> = ({
  ...
  // Tracks whether THIS component instance currently holds the global
  // export lock, so an unmount mid-export (tab closed, navigated away)
  // releases it instead of leaving every future export attempt — even
  // from a different step's panel — permanently blocked.
  const holdsExportLockRef = useRef(false);

  useEffect(() => {
    return () => {
      if (holdsExportLockRef.current) {
        endGlobalExport();
        holdsExportLockRef.current = false;
      }
    };
  }, []);
  ...
  const handleExportVideo = async () => {
    ...
    // Global lock: this app has 4 separate preview panels (one per wizard
    // step) all sharing one audio engine underneath. Without this check, a
    // user could click Export on one step, navigate to another, and click
    // Export there too — running two exports concurrently, both fighting
    // over the same shared AudioContext, producing corrupted output stuck
    // on a single frame. Check this BEFORE spending a credit, so a blocked
    // attempt never costs the user anything.
    if (!beginGlobalExport()) {
      setExportError(
        'An export is already in progress (possibly from another step). Please wait for it to finish before starting another.'
      );
      return;
    }
    holdsExportLockRef.current = true;
