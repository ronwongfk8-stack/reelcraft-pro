...
interface WelcomeLandingPageProps {
  ...
  onOpenHelp?: (tab: 'help' | 'terms' | 'privacy') => void;
}
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950 antialiased">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-16 space-y-12">
        ...
        {/* Hero Promo Video — a personal walkthrough/pitch, so it plays
            with sound on click rather than looping muted in the
            background (which would defeat the point of a talking video). */}
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-center text-xs text-slate-500 mt-3">
            A quick walkthrough of what ReelCraft PRO can do for your marketing videos.
          </p>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800/80 py-6 flex flex-col items-center gap-2 text-center text-xs text-slate-400">
        <p>ReelCraft PRO &bull; Turn Photos into Stunning AI Marketing Videos</p>
        {onOpenHelp && (
          <div className="flex items-center gap-3 text-slate-500">
            <button onClick={() => onOpenHelp('help')} className="hover:text-amber-400 transition-colors">
              Help &amp; Contact
            </button>
            <span>&bull;</span>
            <button onClick={() => onOpenHelp('terms')} className="hover:text-amber-400 transition-colors">
              Terms of Service
            </button>
            <span>&bull;</span>
            <button onClick={() => onOpenHelp('privacy')} className="hover:text-amber-400 transition-colors">
              Privacy Policy
            </button>
          </div>
        )}
