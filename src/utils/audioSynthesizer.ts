...
// This app renders multiple independent <VideoPreview> instances — one per
// wizard step — and they all share the single audioEngine above. Without
// this guard, clicking Export on one step, then navigating to another step
// and clicking Export there too, would let two exports run concurrently,
// both fighting over the same shared AudioContext. That produces exactly
// the kind of corrupted, stuck-on-one-frame output this was built to
// prevent: a global (not per-component) lock that any instance can check.
let globalExportInProgress = false;

export function isExportInProgress(): boolean {
  return globalExportInProgress;
}

export function beginGlobalExport(): boolean {
  if (globalExportInProgress) return false;
  globalExportInProgress = true;
  return true;
}

export function endGlobalExport(): void {
