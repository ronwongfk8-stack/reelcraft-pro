...
// Generates a real voiceover audio track via the backend's Gemini native
// TTS endpoint (gemini-2.5-flash-preview-tts). Replaces the old approach
// (a raw oscillator faking the *rhythm* of speech, not actual words) with
// genuine synthesized speech, matching what the live preview and the
// exported video will both actually sound like — no more mismatch between
// what you hear while editing and what ends up in the download.
export const generateAiVoiceoverAudioBlob = async (
  ...
  // Convert the base64 WAV the backend returns into a playable Blob URL.
